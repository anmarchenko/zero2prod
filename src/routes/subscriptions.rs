...
pub async fn subscribe(
    ...
    let mut transaction = pool
        .begin()
        .await
        .map_err(|e| SubscribeError::UnexpectedError(Box::new(e), "Pool error".into()))?;

    let subscriber_id = insert_subscriber(&mut transaction, &new_subscriber)
        .await
        .map_err(|e| {
            SubscribeError::UnexpectedError(Box::new(e), "Insert subscriber error".into())
        })?;
    let subscription_token = generate_subscription_token();

    store_token(&mut transaction, subscriber_id, &subscription_token)
        .await
        .map_err(|e| SubscribeError::UnexpectedError(Box::new(e), "Store token error".into()))?;

    transaction.commit().await.map_err(|e| {
        SubscribeError::UnexpectedError(Box::new(e), "transaction commit error".into())
    })?;
    // COMMIT
    ...
    .await
    .map_err(|e| {
        SubscribeError::UnexpectedError(Box::new(e), "send confirmation email error".into())
    })?;
